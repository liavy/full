<%@ Page language="c#" Codebehind="MembersGrid.cs" AutoEventWireup="false" Inherits="Book_Store.MembersGrid" %>
...
<asp:HyperLink id=Members_member_login NavigateUrl='<%# "MembersInfo.aspx"+"?"+"member_id="+Server.UrlEncode(DataBinder.Eval(Container.DataItem, "m_member_id").ToString()) +"&" +"name=" + Server.UrlEncode(Utility.GetParam("name")) + "&"%>' style="font-size: 10pt; color: #000000" runat="server"> <%#Server.HtmlEncode(DataBinder.Eval(Container.DataItem, "m_member_login").ToString()) %> </asp:HyperLink>&nbsp;
</td>
<td style="background-color: #FFFFFF; border-width: 1">
